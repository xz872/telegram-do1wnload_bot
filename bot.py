...
async def handle_download(message: types.Message):
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # 1. إرسال للمستخدم مع اليوزر والزر الشفاف
            await send_media(message.chat.id, f"- {BOT_USERNAME}", keyboard)

            # 2. إرسال للوجات بالتنسيق القديم (ID + User + Link)
            log_text = (
                f"📥 تحميل جديد:\n"
        ...
        await status_msg.edit_text("❌ فشل التحميل. الرابط قد يكون خاصاً أو غير مدعوم.")
